...
pub struct Proxy<T: 'static> {
    ...
    // PATCHED: unbounded. With a bounded channel, `free_slots` silently drops
    // acks when this fills; once 100 slots leak, the worker below stops
    // forwarding forever and every subscription/task output wedges while the
    // UI stays responsive. Acks are a usize per event-loop batch, so the
    // queue stays tiny; it must simply never lose one.
...
impl<T: 'static> Proxy<T> {
    ...
    pub fn free_slots(&mut self, amount: usize) {
        // Can only fail when the worker is gone (shutdown); never drops an
        // ack under load.
